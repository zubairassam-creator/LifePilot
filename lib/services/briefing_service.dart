...
class BriefingService {
  static Future<BriefingData> loadTodayBriefing() async {
    ...
    try {
      ...
      developer.log(
        'Failed to load task briefing data',
        error: error,
    ...
    try {
      ...
      birthdaysAndEvents = memories
          .where(
            (memory) =>
                (memory.type == LifeMemoryType.birthday ||
                    memory.type == LifeMemoryType.event) &&
      ...
      openLoans = memories
          .where(
            (memory) =>
                memory.type == LifeMemoryType.loanTaken &&
                memory.status == LifeMemoryStatus.open,
          )
          .toList();
      _sortMemoriesByDate(expiringSoon, (memory) => memory.dueDate);
      _sortMemoriesByDate(
        birthdaysAndEvents,
        (memory) => _nextOccurrence(memory.eventDate!, today),
      );
      ...
      developer.log(
        'Failed to load memory briefing data',
        error: error,
        stackTrace: stackTrace,
      );
    }
  ...
  static String buildSpokenBriefing(BriefingData data) {
    ...
    if (data.allFailed)
      return '$greeting. I could not load your briefing right now. Please try again.';
    if (data.isEmpty)
      return '$greeting. Here is your briefing for today. You are all caught up today.';

    final parts = <String>['$greeting. Here is your briefing for today.'];
    parts.add(
      'You have ${_taskCountPhrase(data.overdue.length, 'overdue')} and ${_scheduledTodayPhrase(data.today.length)}.',
    );
    if (data.overdue.isNotEmpty)
      parts.add(
        'Your overdue ${_itemWord(data.overdue.length)} ${_isAre(data.overdue.length)} ${_join(data.overdue.map((task) => task.title))}.',
      );
    if (data.today.isNotEmpty)
      parts.add(
        'Today, you need to ${_join(data.today.map(_taskSpokenDetail))}.',
      );
    if (data.upcoming.isNotEmpty)
      parts.add(
        'Coming up soon, you have ${_join(data.upcoming.map(_taskSpokenDetail))}.',
      );
    if (data.expiringSoon.isNotEmpty)
      parts.add(
        'Expiring soon: ${_join(data.expiringSoon.map(_memorySpokenDetail))}.',
      );
    if (data.birthdaysAndEvents.isNotEmpty)
      parts.add(
        'Birthdays and events: ${_join(data.birthdaysAndEvents.map(_memorySpokenDetail))}.',
      );
    if (data.openLoans.isNotEmpty)
      parts.add('Open loans: ${_join(data.openLoans.map(_loanSpokenDetail))}.');
    final emptyMemorySections = [
      data.expiringSoon,
      data.birthdaysAndEvents,
      data.openLoans,
    ].where((items) => items.isEmpty).length;
    if (emptyMemorySections == 3)
      parts.add(
        'You have no upcoming expiries, birthdays, events, or open loans.',
      );
    return parts.join(' ');
  }
  ...
  static void _sortTasks(List<LifePilotTask> tasks) => tasks.sort(
    (a, b) =>
        (a.dueDateTime ?? a.createdAt).compareTo(b.dueDateTime ?? b.createdAt),
  );
  static void _sortMemoriesByDate(
    List<LifeMemory> memories,
    DateTime? Function(LifeMemory) date,
  ) => memories.sort(
    (a, b) => (date(a) ?? a.updatedAt).compareTo(date(b) ?? b.updatedAt),
  );
  static bool _occursInNextDays(DateTime eventDate, DateTime today, int days) =>
      !_nextOccurrence(eventDate, today).isBefore(today) &&
      _nextOccurrence(
        eventDate,
        today,
      ).isBefore(today.add(Duration(days: days + 1)));
  static DateTime _nextOccurrence(DateTime date, DateTime today) {
    final occurrence = DateTime(today.year, date.month, date.day);
    return occurrence.isBefore(today)
        ? DateTime(today.year + 1, date.month, date.day)
        : occurrence;
  }

  static String _taskSpokenDetail(LifePilotTask task) =>
      task.dueDateTime == null
      ? task.title
      : '${task.title} at ${_formatTime(task.dueDateTime!)}';
  static String _memorySpokenDetail(LifeMemory memory) =>
      memory.dueDate == null && memory.eventDate == null
      ? memory.title
      : '${memory.title} on ${_formatDate(memory.dueDate ?? memory.eventDate!)}';
  static String _loanSpokenDetail(LifeMemory memory) =>
      '${memory.currency ?? ''}${_formatAmount(memory.amount)} from ${memory.person ?? memory.title}${memory.dueDate == null ? '' : ', due ${_formatDate(memory.dueDate!)}'}';
  static String _taskCountPhrase(int count, String label) => count == 0
      ? 'no $label tasks'
      : count == 1
      ? 'one $label task'
      : '$count $label tasks';
  static String _scheduledTodayPhrase(int count) => count == 0
      ? 'no tasks scheduled for today'
      : count == 1
      ? 'one task scheduled for today'
