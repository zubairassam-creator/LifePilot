...
class DocumentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
    child: ListTile(
      ...
      subtitle: Text('${document.category.label} • ${_date(document.createdAt)}'),
      trailing: document.isSensitive ? const Icon(Icons.lock_outline, size: 20) : null,
      onTap: onTap,
    ),
  );
  IconData get _icon => document.mimeType.startsWith('image/') ? Icons.image_outlined : (document.extension == 'pdf' ? Icons.picture_as_pdf_outlined : Icons.description_outlined);
