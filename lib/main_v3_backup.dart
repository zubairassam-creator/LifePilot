  ...
  Future<void> createReminder() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Could not schedule notification. Please choose a future time and check notification permissions.',
          ),
        ),
      );
    }
  ...
  Future<void> toggleCompleted(int index) async {
    ...
    if (reminder.isCompleted) {
      ...
      try {
        ...
        // A past reminder cannot be scheduled again.
      }
    }
      ...
      SnackBar(
        ...
        action: SnackBarAction(
          ...
          onPressed: () async {
            ...
            if (!deletedReminder.isCompleted) {
              try {
                await NotificationService.scheduleReminder(deletedReminder);
              } catch (_) {
                // A past reminder cannot be rescheduled.
              }
            }
...
class _CreateReminderScreenState extends State<CreateReminderScreen> {
  ...
  void saveReminder() {
    ...
    final int reminderId = DateTime.now().microsecondsSinceEpoch.remainder(
      2147483647,
    );
